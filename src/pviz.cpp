...
PViz::PViz(const std::string &ns, const std::string &ref_frame) {
  ...
  // arm meshes
  arm_meshes_.push_back(
      "package://pr2_description/meshes/shoulder_v0/shoulder_pan.dae");
  arm_meshes_.push_back(
      "package://pr2_description/meshes/shoulder_v0/shoulder_lift.dae");
  arm_meshes_.push_back(
      "package://pr2_description/meshes/shoulder_v0/upper_arm_roll.dae");
  arm_meshes_.push_back(
      "package://pr2_description/meshes/upper_arm_v0/upper_arm.dae");
  arm_meshes_.push_back(
      "package://pr2_description/meshes/upper_arm_v0/elbow_flex.dae");
  arm_meshes_.push_back(
      "package://pr2_description/meshes/upper_arm_v0/forearm_roll.stl");
  arm_meshes_.push_back(
      "package://pr2_description/meshes/forearm_v0/forearm.dae");
  arm_meshes_.push_back(
      "package://pr2_description/meshes/forearm_v0/wrist_flex.dae");
  arm_meshes_.push_back(
      "package://pr2_description/meshes/forearm_v0/wrist_roll.stl");
  arm_meshes_.push_back(
      "package://pr2_description/meshes/gripper_v0/gripper_palm.dae");

  // gripper meshes
  gripper_meshes_.push_back(
  ...
  // torso_meshes
  torso_meshes_.push_back(
      "package://pr2_description/meshes/torso_v0/torso_lift.dae");

  // base meshes
  base_meshes_.push_back("package://pr2_description/meshes/base_v0/base.dae");

  // head meshes
  head_meshes_.push_back(
      "package://pr2_description/meshes/head_v0/head_pan.dae");
  head_meshes_.push_back(
      "package://pr2_description/meshes/head_v0/head_tilt.dae");
  head_meshes_.push_back(
      "package://pr2_description/meshes/tilting_laser_v0/tilting_hokuyo.dae");

  robot_meshes_.insert(robot_meshes_.end(), base_meshes_.begin(),
                       base_meshes_.end());  // 1
  robot_meshes_.insert(robot_meshes_.end(), torso_meshes_.begin(),
                       torso_meshes_.end());  // 1
  robot_meshes_.insert(robot_meshes_.end(), arm_meshes_.begin(),
                       arm_meshes_.end());  // 10
  robot_meshes_.insert(robot_meshes_.end(), gripper_meshes_.begin(),
                       gripper_meshes_.end());  // 4
  robot_meshes_.insert(robot_meshes_.end(), arm_meshes_.begin(),
                       arm_meshes_.end());  // 10
  robot_meshes_.insert(robot_meshes_.end(), gripper_meshes_.begin(),
                       gripper_meshes_.end());  // 4
  robot_meshes_.insert(robot_meshes_.end(), head_meshes_.begin(),
                       head_meshes_.end());  // 3

  if (!initKDLChain()) {
    ROS_ERROR(
        "[pviz] Failed to initiliaze the KDL chain. This should exit but "
        "instead it will crash.");
    fflush(stdout);
  }
...
PViz::~PViz() {
  for (size_t i = 0; i < fk_rsolver_.size(); ++i) delete fk_rsolver_[i];
  for (size_t i = 0; i < fk_lsolver_.size(); ++i) delete fk_lsolver_[i];
}
...
bool PViz::initKDLChain() {
  ...
  if (!nh_.searchParam("robot_description", robot_param)) {
    ROS_ERROR(
        "[pviz] Unable to find the robot_description on the param server.");
    return false;
  }
  ...
  if (!kdl_tree_.getChain("base_footprint", RIGHT_CHAIN_RTIP_NAME, chain_)) {
    ROS_ERROR(
        "Error: could not fetch the KDL chain for the desired manipulator. "
        "Exiting.");
    return false;
  }
  fk_rsolver_[RIGHT] = new KDL::ChainFkSolverPos_recursive(chain_);
  ROS_DEBUG(
      "[pviz] the right arm-right finger chain has %d segments with %d joints",
  ...
  ROS_DEBUG(
      "[pviz] the right arm-left finger chain has %d segments with %d joints",
      chain_.getNrOfSegments(), chain_.getNrOfJoints());
  ...
  ROS_DEBUG(
      "[pviz] the left arm-right finger chain has %d segments with %d joints",
      chain_.getNrOfSegments(), chain_.getNrOfJoints());
  ...
  ROS_DEBUG("[pviz] jnt_pos_in: rows: %d cols: %d", jnt_pos_in_.rows(),
            jnt_pos_out_.columns());
  ...
  ROS_DEBUG("[pviz] the head chain has %d segments with %d joints",
            chain_.getNrOfSegments(), chain_.getNrOfJoints());
  ...
  if (!kdl_tree_.getChain("base_footprint", "laser_tilt_link", chain_)) {
    ROS_ERROR(
        "Error: could not fetch the KDL chain for the tilt laser. Exiting.");
    return false;
  }
  fk_tsolver_ = new KDL::ChainFkSolverPos_recursive(chain_);
  ROS_DEBUG("[pviz] the tilt laser chain has %d segments with %d joints",
            chain_.getNrOfSegments(), chain_.getNrOfJoints());
...
bool PViz::computeFKwithKDL(const std::vector<double> &angles,
  ...
  if (arm == HEAD) {
    ...
    if (base_pos.size() > 3) {
      ...
      if (base_pos.size() > 4) head_pos_in(2) = base_pos[4];  // tilt
    }

    if (fk_hsolver_->JntToCart(head_pos_in, frame_out, frame_num) < 0) {
      ROS_ERROR("JntToCart returned < 0. Exiting. (arm: %d  frame: %d)", arm,
                frame_num);
      return false;
    }
  } else if (arm == TILT)  // tilt laser
  {
    ...
    tilt_pos_in(1) = 0;
    if (base_pos.size() > 5) tilt_pos_in(1) = base_pos[5];  // tilt mount
  ...
  } else  // body
  {
    ...
    for (size_t i = 0; i < angles.size(); ++i) jnt_pos_in_(i + 1) = angles[i];
    ...
    if (frame_num > 0) {
      ...
      if (fk_lsolver_[arm]->JntToCart(jnt_pos_in_, frame_out, -1 * frame_num) <
          0) {
        ROS_ERROR("JntToCart returned < 0. Exiting. (arm: %d  frame: %d)", arm,
                  frame_num);
        return false;
  ...
  frame_out.M.GetQuaternion(pose.orientation.x, pose.orientation.y,
                            pose.orientation.z, pose.orientation.w);
  return true;
}

void PViz::visualizeObstacles(
  ...
  for (size_t i = 0; i < obstacles.size(); ++i)
    ma.markers[i] = viz::getCubeMarker(obstacles[i], color, reference_frame_,
                                       "obstacles", i);

  publish(ma);
}

void PViz::getCubeMsg(std::vector<double> &cube, std::vector<double> &color,
                      std::string ns, int id,
                      visualization_msgs::Marker &marker) {
  marker = viz::getCubeMarker(cube, color, reference_frame_, ns, id);
}

void PViz::getCubeMsg(geometry_msgs::Pose &pose, std::vector<double> &dim,
                      std::vector<double> &color, std::string ns, int id,
                      visualization_msgs::Marker &marker) {
...
void PViz::visualizePoses(const std::vector<std::vector<double> > &poses) {
  visualization_msgs::MarkerArray ma =
      viz::getPosesMarkerArray(poses, reference_frame_, "poses", 0);
  publish(ma);
}
...
void PViz::visualizePose(const geometry_msgs::Pose &pose, std::string text,
                         std::string frame_id) {
  visualization_msgs::MarkerArray ma =
      viz::getPoseMarkerArray(pose, frame_id, text, 0);
  publish(ma);
}

void PViz::visualizeSphere(std::vector<double> pose, int color, std::string ns,
                           double radius) {
  visualization_msgs::Marker m = viz::getSphereMarker(
      pose[0], pose[1], pose[2], radius, color, reference_frame_, ns, 1);
  publish(m);
}

void PViz::visualizeSphere(double x, double y, double z, double radius, int hue,
                           std::string ns, int id) {
  visualization_msgs::Marker m =
      viz::getSphereMarker(x, y, z, radius, hue, reference_frame_, ns, 0);
  publish(m);
}

void PViz::visualizeSpheres(const std::vector<std::vector<double> > &pose,
                            int color, std::string ns, double radius) {
  visualization_msgs::Marker m =
      viz::getSpheresMarker(pose, radius, color, reference_frame_, ns);
  publish(m);
}

void PViz::visualizeSpheres(const std::vector<std::vector<double> > &pose,
                            int color, std::string ns,
                            std::vector<double> &radius) {
  visualization_msgs::MarkerArray ma =
      viz::getSpheresMarkerArray(pose, radius, color, reference_frame_, ns, 0);
  publish(ma);
}

void PViz::visualizeSpheres(const std::vector<std::vector<double> > &pose,
                            int color, std::string ns) {
  if (pose.empty() || pose[0][3]) return;

  visualization_msgs::Marker m =
      viz::getSpheresMarker(pose, pose[0][3], color, reference_frame_, ns, 0);
  publish(m);
}

void PViz::visualizeSpheres(const std::vector<std::vector<double> > &pose,
                            const std::vector<int> &hue, std::string ns) {
  visualization_msgs::MarkerArray ma =
      viz::getSpheresMarkerArray(pose, hue, reference_frame_, ns, 0);
  publish(ma);
}
...
void PViz::visualize3DPath(std::vector<std::vector<double> > &dpath) {
  ...
  for (int k = 0; k < int(dpath.size()); k++) {
    if (int(dpath[k].size()) < 3) continue;
...
void PViz::visualizeBasicStates(const std::vector<std::vector<double> > &states,
  ...
  if (states.empty()) {
    ROS_DEBUG("[pviz] There are no states in the %s states list.",
              name.c_str());
    return;
  }
  ...
  publish(marker);
  ROS_DEBUG("[visualizeBasicStates] published %d markers for %s states",
            int(marker.points.size()), name.c_str());
}

void PViz::visualizeDetailedStates(
    const std::vector<std::vector<double> > &states,
    const std::vector<std::vector<double> > &color, std::string name,
    double size) {
  unsigned int inc = 1;
  ...
  if (states.empty()) {
    ...
    ROS_INFO("[aviz] There are %i states in the %s states list.",
             int(states.size()), name.c_str());
  ...
  */
  ...
  for (unsigned int i = 0; i < states.size(); i = i + inc) {
    ...
    for (unsigned int j = 0; j < 4; ++j)
      scaled_color[j] =
          color[0][j] - ((color[0][j] - color[1][j]) *
                         (double(i) / double(states.size() / inc)));
  ...
  ROS_DEBUG("[aviz] published %d markers for %s states",
            (int)marker_array.markers.size(), name.c_str());
  publish(marker_array);
}

void PViz::visualizeLine(const std::vector<geometry_msgs::Point> points,
                         std::string ns, int id, int hue, double thickness) {
  visualization_msgs::Marker m =
      viz::getLineMarker(points, thickness, hue, reference_frame_, ns, id);
  publish(m);
}

void PViz::visualizeText(geometry_msgs::Pose pose, std::string text,
                         std::string ns, int id, int hue) {
  visualization_msgs::Marker m =
      viz::getTextMarker(pose, text, 0.2, hue, reference_frame_, ns, id);
  publish(m);
}

void PViz::visualizeText(geometry_msgs::Pose pose, std::string text,
                         std::string ns, int id, std::vector<double> color,
                         double size) {
  visualization_msgs::Marker m =
      viz::getTextMarker(pose, text, size, color, reference_frame_, ns, id);
  publish(m);
}

void PViz::visualizeCube(geometry_msgs::PoseStamped pose, int hue,
                         std::string ns, int id, std::vector<double> dim) {
  visualization_msgs::Marker m = viz::getCubeMarker(pose, dim, hue, ns, id);
  publish(m);
}

void PViz::visualizeMesh(const std::string &mesh_resource,
                         const geometry_msgs::PoseStamped &pose, int color,
                         std::string ns, int id) {
  visualization_msgs::Marker m =
      viz::getMeshMarker(pose, mesh_resource, color, ns, id);
  publish(m);
}

void PViz::visualizeMeshTriangles(
    const std::vector<geometry_msgs::Point> &vertices,
    const std::vector<int> &triangles, const geometry_msgs::PoseStamped &pose,
    int color, std::string ns, int id, bool psychadelic) {
  visualization_msgs::Marker m =
      viz::getMeshMarker(pose, vertices, triangles, color, psychadelic, ns, id);
  publish(m);
}

bool PViz::computeFKforVisualizationWithKDL(
    const std::vector<double> &jnt0_pos, std::vector<double> &jnt1_pos,
    std::vector<double> &base_pos, double torso_pos,
    std::vector<geometry_msgs::PoseStamped> &poses) {
  // output is PoseStamped only so it has same interface as
  // computeFKforVisualization
  // KDL::Frame map_to_robot;
  geometry_msgs::Pose pose;

  /*
  [ INFO]:   frame  0: segment: 0.000 0.000 0.051  joint: 0.000 0.000 0.000
  joint_type: None
  [ INFO]:   frame  1: segment: -0.050 0.000 0.740  joint: -0.050 0.000 0.740
  joint_type: TransAxis
  [ INFO]:   frame  2: segment: 0.000 0.188 0.000  joint: 0.000 0.188 0.000
  joint_type: RotAxis
  [ INFO]:   frame  3: segment: 0.100 0.000 0.000  joint: 0.100 0.000 0.000
  joint_type: RotAxis
  [ INFO]:   frame  4: segment: 0.000 0.000 0.000  joint: 0.000 0.000 0.000
  joint_type: RotAxis
  [ INFO]:   frame  5: segment: 0.000 0.000 0.000  joint: 0.000 0.000 0.000
  joint_type: None
  [ INFO]:   frame  6: segment: 0.400 0.000 0.000  joint: 0.400 0.000 0.000
  joint_type: RotAxis
  [ INFO]:   frame  7: segment: 0.000 0.000 0.000  joint: 0.000 0.000 0.000
  joint_type: RotAxis
  [ INFO]:   frame  8: segment: 0.000 0.000 0.000  joint: 0.000 0.000 0.000
  joint_type: None
  [ INFO]:   frame  9: segment: 0.321 0.000 0.000  joint: 0.321 0.000 0.000
  joint_type: RotAxis
  [ INFO]:   frame 10: segment: 0.000 0.000 0.000  joint: 0.000 0.000 0.000
  joint_type: RotAxis
  [ INFO]:   frame 11: segment: 0.000 0.000 0.000  joint: 0.000 0.000 0.000
  joint_type: None
  [ INFO]:   frame 12: segment: 0.077 0.010 0.000  joint: 0.077 0.010 0.000
  joint_type: RotAxis
  [ INFO]:   frame 13: segment: 0.091 0.005 0.000  joint: 0.091 0.005 0.000
  joint_type: RotAxis
  ...
  */
  ...
  for (int i = 0; i < int(poses.size()); i++) {
    ...
    if (i < 14) {
      if (!computeFKwithKDL(jnt0_pos, base_pos, torso_pos, RIGHT, i + 1,
                            poses[i].pose))
        return false;
    ...
    // right arm - left finger only
      ...
      if (!computeFKwithKDL(jnt0_pos, base_pos, torso_pos, RIGHT, -1 * (i - 1),
                            poses[i].pose))
        return false;
    }
    // left arm thru the right finger
    else if (15 < i && i < 28) {
      if (!computeFKwithKDL(jnt1_pos, base_pos, torso_pos, LEFT, i - 13,
                            poses[i].pose))
        return false;
    ...
    // left arm - left finger only
      ...
      if (!computeFKwithKDL(jnt1_pos, base_pos, torso_pos, LEFT, -1 * (i - 15),
                            poses[i].pose))
        return false;
    }
    // head pan, head tilt
    else if (29 < i && i < 32) {
      if (!computeFKwithKDL(jnt1_pos, base_pos, torso_pos, HEAD, i - 27,
                            poses[i].pose))
        return false;
    }
    // tilt laser
    else if (31 < i) {
      if (!computeFKwithKDL(jnt1_pos, base_pos, torso_pos, TILT, i - 29,
                            poses[i].pose))
        return false;
    }
  }
...
visualization_msgs::MarkerArray PViz::getRobotMeshesMarkerMsg(
    double hue, std::string ns, int id,
    std::vector<geometry_msgs::PoseStamped> &poses,
    bool use_embedded_materials) {
  double r, g, b;
  marker_array_.markers.clear();
...
void PViz::getMaptoRobotTransform(double x, double y, double theta,
                                  KDL::Frame &frame) {
  KDL::Rotation r1;
  r1.DoRotZ(theta);
...
void PViz::visualizeRobot(std::vector<double> &jnt0_pos,
                          std::vector<double> &jnt1_pos,
                          std::vector<double> &base_pos, double torso_pos,
                          double hue, std::string ns, int id,
                          bool use_embedded_materials) {
  std::vector<geometry_msgs::PoseStamped> poses;
  if (!computeFKforVisualizationWithKDL(jnt0_pos, jnt1_pos, base_pos, torso_pos,
                                        poses))
...
void PViz::visualizeRobot(std::vector<double> &jnt0_pos,
                          std::vector<double> &jnt1_pos, BodyPose &body_pos,
                          double hue, std::string ns, int id,
                          bool use_embedded_materials) {
  double torso_pos;
...
visualization_msgs::MarkerArray PViz::getRobotMarkerMsg(
    std::vector<double> &jnt0_pos, std::vector<double> &jnt1_pos,
  ...
  if (!computeFKforVisualizationWithKDL(jnt0_pos, jnt1_pos, base_pos, torso_pos,
                                        poses)) {
    ROS_WARN(
        "[pviz] Failed to compute forward kinematics. Cannot visualize robot.");
    return visualization_msgs::MarkerArray();
  } else
    return getRobotMeshesMarkerMsg(hue, ns, id, poses, use_embedded_materials);
}

bool PViz::parseCSVFile(std::string filename, int num_cols,
  ...
  for (int i = 0; i < int(raw_data.size()); i++) {
    if (raw_data[i] != zero_line) data.push_back(raw_data[i]);
  }

  ROS_DEBUG("[pviz] raw_data: num rows: %d", (int)raw_data.size());
  ROS_DEBUG("[pviz] data: num rows: %d num_cols: %d", (int)data.size(),
            int(data[0].size()));
  return true;
}
...
bool PViz::visualizeTrajectoryFromFile(std::string filename,
  ...
  for (size_t i = 0; i < traj.size(); ++i) {
    ...
    visualizeRobot(jnt0_pos, jnt1_pos, base_pos, torso, (30 * i) % 300,
                   "full_body_waypoint_" + boost::lexical_cast<std::string>(i),
                   i, use_embedded_materials);
    ros::spinOnce();
    usleep(10000);
  }
  return true;
...
void PViz::visualizeRobotWithTitle(std::vector<double> &jnt0_pos,
                                   std::vector<double> &jnt1_pos,
                                   BodyPose &body_pos, double hue,
                                   std::string ns, int id, std::string title) {
...
void PViz::visualizeTrajectory(
    std::vector<trajectory_msgs::JointTrajectoryPoint> &rpath,
    std::vector<trajectory_msgs::JointTrajectoryPoint> &lpath,
    std::vector<trajectory_msgs::JointTrajectoryPoint> &bpath, int throttle,
    std::string ns, int id) {
  ...
  if (rpath.size() != lpath.size() || rpath.size() != bpath.size()) {
    ROS_ERROR(
        "[pviz] The right arm, left arm and body trajectories are of unequal "
        "lengths.");
    return;
  }

  int color_inc = 240.0 / (length / throttle);  // hue: red -> blue
  ROS_DEBUG("[pviz] length: %d color_inc: %d throttle: %d)", length, color_inc,
            throttle);
  ...
  for (int i = 0; i < length; ++i) {
    ...
    if ((i != length - 1) && (i % throttle != 0)) continue;

    ROS_DEBUG("[pviz] length: %d color_inc: %d throttle: %d", length, color_inc,
              throttle);
    ROS_DEBUG(
        "[pviz] Visualizing waypoint #%d (i mod color_inc: %d) with color: %d "
        "(color_inc: %d, throttle: %d)",
        i, (i / throttle), (i / throttle) * color_inc, color_inc, throttle);

    ma1 = getRobotMarkerMsg(rangles, langles, body_pos,
                            (i / throttle) * color_inc, ns, id + (i + 1) * 30);
    ma.markers.insert(ma.markers.end(), ma1.markers.begin(), ma1.markers.end());
  }
  ROS_INFO("[pviz] Visualizing a robot path with %d waypoints. (throttle = %d)",
           int(rpath.size()), throttle);
  publish(ma);
}

void PViz::visualizeSpheres(const std::vector<geometry_msgs::Point> &poses,
                            int hue, std::string ns, int id, double radius) {
  visualization_msgs::Marker m =
      viz::getSpheresMarker(poses, radius, hue, reference_frame_, ns, id);
  publish(m);
}

void PViz::visualizeGripper(const geometry_msgs::Pose &pose, double hue,
                            std::string ns, int id, bool open) {
  visualization_msgs::MarkerArray m;
  getGripperMeshesMarkerMsg(pose, hue, ns, id, open, m.markers);
  publish(m);
}

void PViz::getGripperMeshesMarkerMsg(
    const geometry_msgs::Pose &pose, double hue, std::string ns, int id,
    bool open, std::vector<visualization_msgs::Marker> &markers) {
  ...
  g1.position.y = -0.010;
  g1.position.z = 0.00;
  g1.orientation.x = 0.0;
  g1.orientation.y = 0.0;
  g1.orientation.z = -0.231;
  g1.orientation.w = 0.973;
  leatherman::multiply(g1, rot, g1);
  // g2 = r_gripper_l_finger_link in r_gripper_palm - OPEN
  g2.position.x = 0.077;
  g2.position.y = 0.010;
  g2.position.z = 0.00;
  g2.orientation.x = 0.0;
  g2.orientation.y = 0.0;
  g2.orientation.z = 0.231;
  g2.orientation.w = 0.973;
  // g2 = g1; g2.position.y = 0.010; g2.orientation.z = 0.231;
  // g3 = r_gripper_r_finger_tip_link in r_gripper_palm - OPEN
  g3.position.x = 0.156;
  g3.position.y = -0.056;
  g3.position.z = 0.00;
  g3.orientation.x = 0.0;
  g3.orientation.y = 0.0;
  g3.orientation.z = 0.0;
  g3.orientation.w = 1.000;
  leatherman::multiply(g3, rot, g3);
  // g4 = r_gripper_l_finger_tip_link in r_gripper_palm - OPEN
  g4.position.x = 0.156;
  g4.position.y = 0.056;
  g4.position.z = 0.00;
  g4.orientation.x = 0.0;
  g4.orientation.y = 0.0;
  g4.orientation.z = 0.0;
  g4.orientation.w = 1.000;
  // g4 = g3; g4.position.y = 0.056;
  // g5 = r_gripper_r_finger_link in r_gripper_palm - CLOSED
  g5.position.x = 0.077;
  g5.position.y = -0.010;
  g5.position.z = 0.00;
  g5.orientation.x = 0.0;
  g5.orientation.y = 0.0;
  g5.orientation.z = 0.004;
  g5.orientation.w = 1.000;
  // g5 = g1; g5.orientation.x = 0.0; g5.orientation.y = 0.0; g5.orientation.z =
  // 0.004; g5.orientation.w = 1.000;
  leatherman::multiply(g5, rot, g5);
  // g6 = r_gripper_l_finger_link in r_gripper_palm - CLOSED
  g6.position.x = 0.077;
  g6.position.y = 0.010;
  g6.position.z = 0.00;
  g6.orientation.x = 0.0;
  g6.orientation.y = 0.0;
  g6.orientation.z = -0.004;
  g6.orientation.w = 1.000;
  // g6 = g5; g6.position.y = 0.010; g6.orientation.z = -0.004;
  // g7 = r_gripper_r_finger_tip_link in r_gripper_palm - CLOSED
  g7.position.x = 0.168;
  g7.position.y = -0.014;
  g7.position.z = 0.00;
  g7.orientation.x = 0.0;
  g7.orientation.y = 0.0;
  g7.orientation.z = 0.0;
  g7.orientation.w = 1.000;
  leatherman::multiply(g7, rot, g7);
  // g8 = r_gripper_l_finger_tip_link in r_gripper_palm - CLOSED
  g8.position.x = 0.168;
  g8.position.y = 0.014;
  g8.position.z = 0.00;
  g8.orientation.x = 0.0;
  g8.orientation.y = 0.0;
  g8.orientation.z = 0.0;
  g8.orientation.w = 1.000;
...
void PViz::getGripperMeshesMarkerMsg(
    const geometry_msgs::Pose &pose, double hue, std::string ns, int id,
    double position, std::vector<visualization_msgs::Marker> &markers) {
  static geometry_msgs::Pose rot;
  rot.orientation = tf::createQuaternionMsgFromRollPitchYaw(M_PI, 0, 0);
...
void PViz::visualizeText(double x, double y, double z, double size,
                         std::string text, int hue, std::string ns, int id) {
  geometry_msgs::Pose p;
  p.position.x = x;
  p.position.y = y;
  p.position.z = z;
  p.orientation.w = 1.0;
  visualization_msgs::Marker m =
      viz::getTextMarker(p, text, size, hue, reference_frame_, ns, id);
  publish(m);
}
